...
function Marketplace() {
  ...
  function handleImageUpload(event) {
    ...
    setToastMessage(`${selectedFiles.length} imagen${selectedFiles.length > 1 ? 'es' : ''} agregada${selectedFiles.length > 1 ? 's' : ''} al carrusel.`)
    event.target.value = ''
  }

  function handleImageDelete(imageToDelete) {
    setCarouselImages((currentImages) => currentImages.filter((image) => image !== imageToDelete))
    if (imageToDelete.startsWith('blob:')) URL.revokeObjectURL(imageToDelete)
    setToastMessage('Imagen eliminada del carrusel.')
  }
  ...
  return (
    <main className="marketplace-page">
      ...
      <section className="marketplace-content" aria-labelledby="marketplace-featured-title">
        ...
        <div className="marketplace-carousel-area">
          <div className="marketplace-stack" aria-label="Proveedores destacados">
            {carouselImages.length ? <Stack
              randomRotation={false}
              sensitivity={200}
              sendToBackOnClick
              cards={carouselImages.map((src, index) => (
                <img
                  key={src}
                  src={src}
              ...
              autoplayDelay={3000}
              pauseOnHover={false}
            /> : <p className="marketplace-carousel-empty">Agrega imágenes para mostrar el carrusel.</p>}
          </div>
          <div className="marketplace-image-tools">
            <label className="marketplace-upload-button">
              <span aria-hidden="true">+</span> Agregar imágenes
              <input type="file" accept="image/*" multiple onChange={handleImageUpload} />
            </label>
            <p>Gestiona las fotos destacadas de tu comunidad.</p>
            <div className="marketplace-image-list">
              {carouselImages.map((image, index) => (
                <div className="marketplace-image-item" key={image}>
                  <img src={image} alt={`Miniatura ${index + 1}`} />
                  <button type="button" onClick={() => handleImageDelete(image)} aria-label={`Eliminar imagen ${index + 1}`} title="Eliminar imagen">×</button>
                </div>
              ))}
            </div>
          </div>
